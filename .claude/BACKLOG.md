...
| ISS-009 | Issue | 2026-02-20 | Angel One WebSocket never connects at startup | `docker-compose.yaml`, `data_pipeline.py` | P2 — Medium | Fixed | Root cause found 2026-02-26: Angel One SmartWebSocketV2 requires `/app/logs` with write access. Container `start.sh` tried `mkdir -p /app/logs` but failed silently (restricted volume permissions). On WebSocket auth, `SmartWebSocketV2.__init__` called `os.makedirs('logs')` → `PermissionError` → connection pool init failed → `connect()` returned `False` after 5 retries. Fix: added bind mount `./data/openalgo_angelone_broker_logs:/app/logs` in `docker-compose.yaml`. Previous failback bugs (ISS-009 original) were likely downstream of Angel One never connecting in the first place. |
...
| IDEA-006 | Idea | 2026-03-02 | V2: VWAP filter only on first pyramid entry | `continuous_filter.py`, `baseline_v1_live.py` | P2 — Medium | Idea | In V1, the VWAP filter (price >= 4% above VWAP) applies to every entry in a pyramid sequence. This causes 2nd/3rd entries to get rejected as price drifts closer to VWAP, limiting pyramids to 1-2 entries. Proposal: apply VWAP filter only on the first entry of a pyramid sequence — ensures quality starting zone but allows subsequent swing-break entries without the VWAP gate. Other filters (price range 100-300, SL% 2-10%) still apply to all entries. |
| IDEA-007 | Idea | 2026-03-02 | Optimize EC2 data egress to Google Drive | EC2 cron / rclone / Google Drive | P3 — Low | Idea | Current weekly DuckDB backup (~23 MB/week) has low egress cost, but as data grows over months the daily/weekly uploads will increase EC2 data transfer costs. Explore optimizations: incremental/delta backups instead of full dumps, compression improvements, upload frequency reduction, or alternative storage (S3 same-region transfer is free). Low priority — revisit when data size becomes meaningful. |
