...
class LoginHandlerV3:
    ...
    def _login_openalgo(self, username: str, password: str) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return True
            logger.error(f"[RL-V1-LOGIN] OpenAlgo login failed: {response.status_code}")
        except Exception as e:
    ...
    def auto_login(self, user_id: str, password: str, totp_secret: str,
        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                if r.status_code == 200:
                    logger.info(f"[RL-V1-LOGIN] Upstox OpenAlgo is ready (attempt {attempt})")
                    break
            except Exception:
                pass
            if attempt < max_retries:
                logger.info(f"[RL-V1-LOGIN] Waiting for Upstox OpenAlgo... ({attempt}/{max_retries})")
        ...
        logger.info(f"[RL-V1-LOGIN] Upstox broker login: user_id={user_id}, TOTP generated")
        logger.warning(
            "[RL-V1-LOGIN] Upstox auto-login callback not yet configured. "
